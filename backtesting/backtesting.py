...
class Trade:
    ...
    def close(self, portion: float = 1.):
        ...
        self.__broker.orders.insert(0, Order(self.__broker, -np.ceil(self.size * portion)))
    ...
    def __set_contingent(self, type, price):
        ...
        if order:
            order.cancel()
        if price:
            kwargs = {'stop' if type == 'sl' else 'limit': price}
...
class _Broker:
    ...
    def _reduce_trade(self, trade: Trade, price: float, size: float, time_index: int):
        ...
        trade._replace(size=trade.size + size)
        if trade._sl_order:
        ...
        part_trade = trade._copy(size=-size, sl_order=None, tp_order=None)
        self.trades.append(part_trade)
        self._close_trade(part_trade, price, time_index)
